		...
		<view v-for="(item,index) in alllist" :key='index' class="goos_lists">
			<view class="space_between">
				<view class="time_clock flex_row">
					...
					<text class="goods_time">2023-6-2 17:15:20 ----订单编号：{{item.id}}</text>
				</view>
				<view class="_state  align_items_center">
					...
					<image v-if="item.state == 4" class="delete_image" src="/static/order/delete.png"
						@click="delectimg(item)"></image>
				</view>
			</view>
