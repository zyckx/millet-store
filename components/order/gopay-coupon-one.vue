		...
		<uni-popup class="gocoupon" :show="coupuptype === 'typecoupup'" position="bottom" @hidePopup="togglePopup('')">
			...
			<view v-if="couponone==2">
				...
				<view class="bottom-content">
					<view class="discountwarp">
						<view class="bank" :class="index === selectbankon ? 'on bank'+index : 'bank'+index"
							@click="selectbank(item,index)" v-for="(item,index) in bank" :key="index">
							{{item.text}}
			...
			<view v-if="couponone==3">
				...
				<view class="bottom-content">
					...
					<view class="grid">
						<view class="gridflex" v-for="(item,index) in pwarr" @click="dotnum(item,index)" :key="index">
							{{item.num}}
						</view>
					</view>
				</view>
